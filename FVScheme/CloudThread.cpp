...
ciphertext_t getxBeta(ciphertext_t* rawDataPoly, ciphertext_t* betaPoly,ciphertext_t labelsPoly, int batchsize, int iter, int quantization, int m){
    //cout<<"jer"<<endl;
	ciphertext_t res;
			if (iter > 0) {
				ciphertext_t inbetween = rawDataPoly[0] * betaPoly[0];
				for (size_t x = 1; x < m - 1; x++) {
					inbetween += rawDataPoly[x]* betaPoly[x];
					
				}

				res = ciphertext_t(inbetween * (24 * quantization));
				res+=  + 50 * quantization * quantization;
				res-= (inbetween * inbetween);//take care of this later on
			}
			
			if(iter==0)
				res= (labelsPoly * 10) -5;
			else
				res =  labelsPoly * (quantization * quantization * ((iter == 0) ? 10 : 100)) 
						-res;
...
ciphertext_t* calculateIteration(int m,ciphertext_t *betaPoly, ciphertext_t **rawDataPoly, ciphertext_t *labelsPoly, int batchsize, unsigned long long int quantization, int iter, pk_t &pk,sk_t &sk) {
	...
	std::future<ciphertext_t> fut[batchsize];
	for(int i=0;i<batchsize;i+=CPU){
		for (int j=0;j<CPU;j++){
			if(i+j<batchsize){
				fut[i+j]=  std::async (getxBeta,rawDataPoly[i+j],betaPoly,labelsPoly[i+j],batchsize,iter,quantization,m);
			}else
				break;
		}
		/*for (int j=0;j<CPU;j++){
			if(i+j<batchsize){
				xbeta[i+j] = fut[i+j].get();
			}else
				break;
		}*/
	}
	//#pragma omp parallel num_threads(CPU)
	{
		...
		for (size_t y = 0; y < m - 1; y++) {
			ciphertext_t inbetween = rawDataPoly[0][y] * fut[0].get();
			for (size_t x = 1; x < batchsize; x++) {
				inbetween = inbetween + rawDataPoly[x][y] * fut[x].get();
			}
			//#pragma omp critical
			if (iter > 0) {
