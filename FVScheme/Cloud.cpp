...
FV::encrypt_integer(five, pk, 5);
    ...
    if (myfile.is_open() && myfile2.is_open()) {
        for (int iter = 0; iter < max_iter; iter++) {
            ...
            std::cout << "Read Encrypted Data from File" << std::endl;

            //cout<<pk.delta<<endl;


            //free(labels);
            //for (unsigned y = 0; y < n; y++)  free(rawData[y]);
	...
	//#pragma omp parallel num_threads(CPU)
		#pragma omp parallel for
            for (unsigned y = start; y < start + batchsize; y++) {
                ...
                if (iter > 0) {
                    ...
                    ciphertext_t inbetween = rawDataPoly[y - start][0] * betaPoly[0];
                    //#pragma omp parallel for
                    for (unsigned x = 1; x < m - 1; x++) {
                        ...
                        inbetween += (rawDataPoly[y - start][x] * betaPoly[x]);
                        //if(iter>1) {
                            /*printEncValPrint(rawDataPoly[y - start][x], sk, pk);
                            cout << "*";
                    ...
                    xbeta[y - start] = inbetween * (24 * quantization );
                    //xbeta[y-start] = inbetween*(twenty4*quantization/hundred);
                    xbeta[y - start] = xbeta[y - start] + 50 * quantization * quantization ;
                    xbeta[y - start] = xbeta[y - start] - (inbetween * inbetween);//take care of this later on
                    

                } else {
                    //FV::encrypt_integer(xbeta[y - start], pk, 5);//.5 to 5
                    xbeta[y-start]=five;
                }
		}
		...
		//#pragma omp parallel for
            for (unsigned y = start; y < start + batchsize; y++) {
                ...
                xbeta[y - start] = labelsPoly[y - start] * (quantization * quantization *((iter == 0) ? 10 : hundred)) -//((iter == 0) ? hundred : 1))
                                   xbeta[y - start];
                /*std::cout<<"xbeta["<<y<<"] ";
                printEncValPrint(xbeta[y - start],sk,pk);
                cout<<endl;*/
            }
            /*ciphertext_t cttt=(xbeta[0]);
            printEncVal(cttt,sk,pk);*/
//#pragma omp parallel num_threads(CPU)
            {
#pragma omp parallel for
            //ciphertext_t tmp1[m - 1];
            for (unsigned y = 0; y < m - 1; y++) {
                ciphertext_t inbetween = rawDataPoly[0][y] * xbeta[0];
                for (unsigned x = start + 1; x < start + batchsize; x++) {
                    inbetween = inbetween + rawDataPoly[x - start][y] * xbeta[x - start];
                }
                ...
                if (iter > 0) {
                    ...
                    betaPoly[y] = betaPoly[y] * (quantization * quantization * 10*hundred) + inbetween;//hundred for alpha
                    /*std::cout << " beta ";
                    printEncValPrint(betaPoly[y] , sk, pk);
                    std::cout << std::endl;*/
                } else
