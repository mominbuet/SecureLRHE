...
namespace FV {
    ...
    //    using poly_c = nfl::poly_from_modulus<uint32_t, 8, 120>;
    //    template <typename T>
    //    struct plaintextModulus;
    //    template <>
    //    struct plaintextModulus<mpz_class> {
    //        static mpz_class value() {
    //            return mpz_class(poly_c::moduli_product());//mpz_class("987654345678987654345678987654323456953");
    //        }
    //    };
...
// @param poly_c (nfl::poly) ***This is new***
...
//
    namespace params {
        ...
        using polyZ_p = nfl::poly<typename poly_t::value_type, poly_t::degree,
                poly_t::nmoduli * 2 + 1>;
        using poly_pl =
        nfl::poly_p<typename poly_c::value_type, poly_c::degree, poly_c::nmoduli>;
        using polyZ_pl = nfl::poly<typename poly_c::value_type, poly_c::degree,
                poly_c::nmoduli * 2 + 1>;
    }
...
namespace FV {
    ...
    public:
        ...
        /// Constructor
        evk_t( size_t word_size): word_size(word_size) {
...
namespace FV {
    ...
    public:
        ...
        pk_t(P const &ai, P const &a_shoupi,P const &bi,P const &b_shoupi,evk_t const &evaluation_key) {
            a = ai;
            b =bi;
            a_shoup = a_shoupi;
            b_shoup = b_shoupi;
 ...
 */
namespace FV {
    params::poly_p to_cipher_poly(params::poly_pl &pt) {
        std::array<mpz_t, params::poly_pl::degree> p;
        for (size_t i = 0; i < params::poly_pl::degree; i++) {
            mpz_init(p[i]);
        }
        pt.poly2mpz(p);
        params::poly_p result;
        result.mpz2poly(p);
        //result.ntt_pow_phi();
    ...
    void encrypt_poly(C &ct, const PK &pk, params::poly_pl &poly_m) {
        using P = params::poly_p;
        //std::cout<<"New encrypt_poly"<<std::endl;
        //Apply inverse NTT on the input cleartext poly
        poly_m.invntt_pow_invphi();
        //Convert plaintext polynomial to the ciphertext space

        P message = to_cipher_poly(poly_m);
        ...
        ct.c0 = ct.c0 + nfl::shoup(u * pk.b, pk.b_shoup) +
                nfl::shoup(message * pk.delta, pk.delta_shoup);
    ...
    void encrypt_poly(C &ct, const PK &pk, params::poly_p &poly_m) {
        using P = params::poly_p;
        //std::cout<<"Old encrypt_poly"<<std::endl;
        // Apply the NTT on poly_m
        poly_m.ntt_pow_phi();
 ...
 */
namespace FV {
    void to_plain_poly(std::array<mpz_t, params::poly_p::degree> &poly_mpz) {
        //std::array<mpz_t, params::poly_p::degree> p;
        //for (size_t i = 0; i < params::poly_p::degree; i++)
        //{
        //    mpz_init(p[i]);
        //    mpz_set_ui(p[i], ct.data()[i]);
        //}

        params::poly_pl result;
        result.mpz2poly(poly_mpz);
        //result.ntt_pow_phi();
    ...
    void decrypt_poly(std::array<mpz_t, params::poly_p::degree> &poly_mpz,
        ...
        util::reduce<P::degree>(
                ...
                pk.evk->qDivBy2);
        if (pack) {
            to_plain_poly(poly_mpz);
            //std::cout<<"Packed decrypt"<<std::endl;
        }
        for (size_t i = 0; i < P::degree; i++) {
